...
#define FILE_OBJECT "../../common_data/wa.object.dat.r.51000"
...
void init_input(){
	...
	map<double, map<double,int> > nmap;
    srand(0);
	...
	while( fscanf(fin, "%d %lf %lf", &nid, &x, &y ) == 3 ){
		...
		while ( nmap[x][y] != 0 ){
			x += (double)(rand() % 100 + 1) / 100000000;
			y += (double)(rand() % 100 + 1) / 100000000;
		} 
...
void knn_query( int src, int K, vector<int>& cands, vector<int> &result ){
	...
	for ( int i = 0 ; i < result.size(); i++ ){
		printf("ID=%d DIS=%lf\n", result[i], get_graph_dis(src,result[i]));
	}
	*/
...
//	exit(0);
	...
	printf("X=[%lf,%lf], Y=[%lf,%lf] Q=%lf\n", min_llx, max_urx, min_lly, max_ury, sqrt((max_urx-min_llx)*(max_urx-min_llx)+(max_ury-min_lly)*(max_ury-min_lly)) );
    ...
    while( o.size() > QUERY_CAND_NUM_OBJECT ){
        o.pop_back();
    }
	*/
/*
	char file_object[100];
    sprintf( file_object, "%s.%d", FILE_OBJECT, QUERY_CAND_NUM_OBJECT );
	...
	while( scanf("%d %d", &locid, &K ) == 2 ){
		TIME_TICK_START
		knn_query(locid, K, o, result);	
		TIME_TICK_END
	    TIME_TICK_PRINT("QUERY");
		for ( int i = 0; i < result.size(); i++ ){
			printf("ID=%d DIS=%d\n", result[i], (int)get_graph_dis(locid, result[i]) );
		}
	}
    ...
    long long total_time_5 = 0; 
    long long total_time_10 = 0; 
    long long total_time_20 = 0; 
    long long total_time_50 = 0; 

    vector<int> query_location;
    
    FILE* fin = fopen( FILE_QUERY, "r" );
    int oid, id;
    while( fscanf(fin, "%d %d", &oid, &id ) == 2 ){ 
        query_location.push_back(oid);
    }    
    fclose(fin);
...
//		if ( k > 9 ) break;
        ...
        printf("NOW AT OBJECT %d / %d\n", k, 100 );
		
		char file_object[100];
		sprintf( file_object, "%s.%d", FILE_OBJECT, k );
...
//		temp = o;	
		...
		for ( int i = 0; i < query_location.size(); i++ ){
//			if ( i > 0 ) break;
			
/*
			o.clear();
			for ( int j = 0; j < temp.size(); j++ ){
				if ( temp[j] != query_location[i] && get_euclidean_dis( temp[j], query_location[i] ) > 5000 ) o.push_back( temp[j] );
			}
			if ( o.size() != 4300 ) count ++;
*/
			object_size ++;


            if ( i % ( query_location.size() / 10 ) == 0 ){ 
                printf("PERCENT: %d / 10\n", i / ( query_location.size() / 10 ) ); 
            }    
            // K = 1
            TIME_TICK_START
      //      knn_query(query_location[i],1,o,result); 
            TIME_TICK_END
            total_time_1 += te - ts;
    
            // K = 5
            TIME_TICK_START
    //        knn_query(query_location[i],5,o,result); 
            TIME_TICK_END
            total_time_5 += te - ts;
...
//			if ( o.size() == 4300 ){
	            TIME_TICK_START
		        knn_query(query_location[k],10,o,result);    
			    TIME_TICK_END
	            total_time_10 += te - ts;
//			}
            ...
            // K = 20
            TIME_TICK_START
  //          knn_query(query_location[i],20,o,result);    
            TIME_TICK_END
            total_time_20 += te - ts;
            
		    // K = 50
            TIME_TICK_START
//            knn_query(query_location[i],50,o,result);
            TIME_TICK_END
            total_time_50 += te - ts;
		}
    }
	printf("TOTAL=%d\n", count );
    printf("TOTAL 1: %lld AVG: %lld\n", total_time_1, total_time_1 / object_size  );
    printf("TOTAL 5: %lld AVG: %lld\n", total_time_5, total_time_5 / object_size  );
    printf("TOTAL 10: %lld AVG: %lld\n", total_time_10, total_time_10 / object_size  );
    printf("TOTAL 20: %lld AVG: %lld\n", total_time_20, total_time_20 / object_size  );
    printf("TOTAL 50: %lld AVG: %lld\n", total_time_50, total_time_50 / object_size  );	
