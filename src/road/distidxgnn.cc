/* ----------------------------------------------------------------------------
    ...
    This program performs kNN object search based on distance index.

    Suggested arguments:
    > (prog name) -i graph.idx -d dist.idx -q #queries -r range -v
    explanations:
    -i: graph index file (input)
    -d: distance index file
    -q: #queries
    -s: #sources
    -k: #NNs
...
int main(const int a_argc, const char** a_argv)
{
    ...
    cerr << "kNN object search on distance index" << endl;
    //-------------------------------------------------------------------------
    // initialization
    //-------------------------------------------------------------------------
    ...
    for (int i=0; i<numquery; i++)
    {
        ...
        DistIndexSearch::groupKNNSearch(
            graph, didx, src, numsrc, k,
