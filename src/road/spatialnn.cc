/* ----------------------------------------------------------------------------
    ...
    This program performs range search on objects based on spatial index.

    Suggested arguments:
    > (prog name) -i graph.idx -o object.dat -q #queries -k k -v
    explanations:
...
int main(const int a_argc, const char** a_argv)
{
    ...
    cerr << "range search on both spatial index and plain graph" << endl;
    //-------------------------------------------------------------------------
    // initialization
    //-------------------------------------------------------------------------
    ...
    for (int i=0; i<numquery; i++)
    {
        ...
        SpatialObjectSearch::kNNSearch(
            graph, smap, src, k, result, nodeaccess, edgeaccess);
        ftime(&endtime);
        float qtime = 
            ((endtime.time*1000 + endtime.millitm) -
            (starttime.time*1000 + starttime.millitm)) / 1000.0f;
